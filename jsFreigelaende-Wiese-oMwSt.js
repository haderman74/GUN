function calc() 
	...
	if (tmpFlaechePlatzmiete < 25)
		{
		tmpKostenPlatzmiete = 120;
		document.getElementById('KostenPlatzmiete').value = Runden2Dezimal(tmpKostenPlatzmiete);
		}
	else
		{
		tmpKostenPlatzmieteAufschlag = (tmpFlaechePlatzmiete - 24) * 1.50	;
		tmpKostenPlatzmiete = tmpKostenPlatzmieteAufschlag + 120;
		document.getElementById('KostenPlatzmiete').value = Runden2Dezimal(tmpKostenPlatzmiete);
		}
	//Kosten für Strom
	...
	//Kosten für Wasser
		...
		tmpKostenWasser = getRadioCheckedValue('Wasserpauschale') * 40;
		document.getElementById('KostenWasser').value = Runden2Dezimal(tmpKostenWasser);
	...
	//Gesamtkosten ermitteln
		...
		tmpKostenGesamt = tmpKostenPlatzmiete + tmpKostenStrom + tmpKostenWasser + tmpKostenTransparent + tmpWerbekostenpflichtbeitrag + tmpNachlassTransparent;
		//tmpMehrwertsteuer = tmpKostenGesamtohne * 20 / 100;
		//tmpKostenGesamt = tmpKostenGesamtohne + tmpMehrwertsteuer;
		//document.getElementById('KostenGesamtohne').value = Runden2Dezimal(tmpKostenGesamtohne);
		//document.getElementById('Mehrwertsteuer').value = Runden2Dezimal(tmpMehrwertsteuer);
